...
def plot_cache_hits_vs_group_num(resultset, topology, alpha, group_num_range, strategy, workloads, rank_sum,  plotdir):
    desc = {}
    desc['title'] = 'Topology:%s Alpha=%s Rank_sum=%s' % (topology, alpha, rank_sum)
    desc['xlabel'] = u'num of group'
    desc['ylabel'] = 'Cache hit ratio'
    desc['xparam'] = ('workload','group_num')
    desc['xvals'] = group_num_range
    desc['filter'] = {'topology': {'name': topology},
        'strategy': {'name': strategy}, 'workload': {'rank_sum': rank_sum}}
        ...
        for topology in topologies:
            for alpha in alphas:
                for strategy in strategies:
                    logger.info('Plotting cache hit ratio for topology %s and alpha %s and strategy %s and rank_sum %s vs group_num' \
                    % (topology, str(alpha), strategy, str(rank_sum)))
                    plot_cache_hits_vs_group_num(resultset, topology, alpha, group_nums, strategy, workloads, rank_sum, plotdir)

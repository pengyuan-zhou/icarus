...
def plot_cache_hits_vs_rank_diff(resultset, topology, alpha, rank_diff_range, strategy, workloads, rank_sum,  plotdir):
    desc = {}
    desc['title'] = 'Cache hit ratio vs diff_rank_diff_ratio'#: T=%s A=%s S=%s R=%s' #% (topology, alpha, strategy, rank_sum)
    desc['xlabel'] = u'rank disimilarity'
    desc['ylabel'] = 'Cache hit ratio'
    desc['xscale'] = 'log'
    desc['xparam'] = ('workload','rank_diff')
    desc['xvals'] = rank_diff_range
    desc['filter'] = {'topology': {'name': topology},
        'strategy': {'name': strategy}, 'workload': {'rank_sum': rank_sum}}
        ...
        for topology in topologies:
            for alpha in alphas:
                for strategy in strategies:
                    logger.info('Plotting cache hit ratio for topology %s and alpha %s and strategy %s and rank_sum %s vs rank_diff' \
                    % (topology, str(alpha), strategy, str(rank_sum)))
                    plot_cache_hits_vs_rank_diff(resultset, topology, alpha, rank_diffs, strategy, workloads, rank_sum, plotdir)

...
def plot_link_load_vs_group_size(resultset, topology, cache_size, group_size_range, strategies, plotdir):
    desc = {}
    desc['title'] = 'Internal link load vs Group Size: T=%s C=%s' % (topology, cache_size)
    desc['xlabel'] = u'Group size \u03b1'
    desc['ylabel'] = 'Internal link load'
    desc['xparam'] = ('toplogy', 'n_member')
    desc['xvals'] = group_size_range
    desc['filter'] = {'topology': {'name': topology},
                      'cache_placement': {'network_cache': cache_size}}
    ...
    desc['legend'] = STRATEGY_LEGEND
    desc['plotempty'] = PLOT_EMPTY_GRAPHS
    plot_lines(resultset, desc, 'LINK_LOAD_INTERNAL_VS_GROUP_SIZET=%s@C=%s.pdf'
               % (topology, cache_size), plotdir)
    ...
    for topology in topologies:
        for alpha in alphas:
            for cache_size in cache_sizes:
                ...
                plot_link_load_vs_group_size(resultset, topology, cache_size, groupsizess, strategies, plotdir)

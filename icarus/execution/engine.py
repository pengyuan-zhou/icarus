...
__all__ = ['exec_experiment', 'test_experiment']
...
def exec_experiment(topology, workload, netconf, strategy, cache_policy, collectors):
    ...
    for time, event in workload:
        ...
        #print "after", topology.node[workload.receiver]
    return collector.results()

def test_experiment(topology, test_workload, netconf, strategy, cache_policy, collectors):
    """Execute the simulation of a specific scenario.
    ...
    strategy_inst = STRATEGY[strategy_name](view, controller, **strategy_args)
    edge_cache_set = dict()
    for time, event in test_workload:
        strategy_inst.process_event(time, **event)
           #add edge_cache to a list,examine if the edge_cache is already in the list
        edge_cache_set.setdefault(strategy_inst.edge_cache,[])
        #print "test", topology.node[test_workload.receiver]
        if test_workload.receiver not in edge_cache_set[strategy_inst.edge_cache]:
            edge_cache_set[strategy_inst.edge_cache].append(test_workload.receiver)

    return edge_cache_set
